...
- `migrations/043_create_github_oauth_connections.sql` creates purpose-specific GitHub Integration OAuth connections with active-account uniqueness and all-deny RLS. It is recorded in Supabase migration history as `20260712160952_043_create_github_oauth_connections`.
- `migrations/044_invalidate_legacy_github_oauth_tokens.sql` invalidates legacy GitHub App and ProjectV2 OAuth credentials during the dev incompatible cutover. It is recorded in Supabase migration history as `20260712160959_044_invalidate_legacy_github_oauth_tokens`.
- `migrations/045_drop_legacy_github_oauth_columns.sql` removes the ten legacy OAuth credential columns from `users` while retaining the GitHub login identity columns. It is recorded in Supabase migration history as `20260712161007_045_drop_legacy_github_oauth_columns`.
