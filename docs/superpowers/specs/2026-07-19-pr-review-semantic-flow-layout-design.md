...
- 새 Canvas와 사용자가 요청한 Flow 자동 정렬에서 1번부터 리뷰 순서대로 왼쪽에서 오른쪽으로 읽히게 한다.
...
`workflowOrder`를 node rank의 유일한 spine으로 사용한다.

- Flow 내부 파일을 `workflowOrder`, file path, stable ID 순으로 정렬한다.
- ELK에는 1→2→3 순서의 내부 layout edge만 전달한다.
- 역할 우선순위와 entry/verification FIRST/LAST constraint는 node rank에 사용하지 않는다.
- semantic relation은 ELK rank 계산에 넣지 않는다.
...
- `review_order` 인접 관계는 가능한 경우 오른쪽 방향의 직선 또는 짧은 orthogonal route를 사용한다.

따라서 semantic relation이 역방향이거나 순환해도 1번 파일이 Flow의 가장 왼쪽에 있고 이후 번호가 오른쪽으로 이어진다. Flow 제목과 설명은 Flow node bounds의 시작점에 붙으므로 1번 파일 위에서 읽기 시작한다.
...
- Dagre에는 semantic relation이 아니라 movable node의 `workflowOrder` 순서를 잇는 내부 edge만 전달한다.
- 고정된 node는 이동하지 않는다.
- 사용자가 자동 정렬을 실행했을 때만 고정되지 않은 node 위치를 저장한다.

고정 node가 리뷰 순서와 다른 위치에 있으면 고정 상태가 우선한다. 이 예외 때문에 기존 Canvas를 로드하는 것만으로 좌표를 자동 수정하지 않는다. 고정되지 않은 일반 Flow에서는 1번부터 순서대로 왼쪽에서 오른쪽으로 배치된다.
...
- Worker-first 배포 순서
- 최초 materialization과 Flow 자동 정렬의 review-order spine 규칙
...
- 역방향·순환 semantic relation이 있어도 최초 배치와 자동 정렬에서 1→2→3 순서가 유지된다.
