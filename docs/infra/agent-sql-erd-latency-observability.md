...
정상적인 집중 보기 요청은 첫 planning turn의 Router/Planner/inspect tool과 다음
planning turn의 Router/Planner/focus tool이 같은 `trace_key`로 이어진다. retry는
AI Worker 이벤트의 `turn_sequence`로 구분한다.

## Dev smoke

1. dev SQLtoERD session에서 새 채팅으로 특정 기능 테이블 집중 보기를 요청한다.
2. AI Worker와 App Server log group의 최근 5분 범위에서 `agent_latency`를 찾는다.
3. 같은 `trace_key`에서 Router, Planner, inspect, 다음 planning turn, focus가
   시간 순서로 이어지는지 확인한다.
4. 모든 `elapsed_ms`가 0 이상이고 Router/Planner의 token 수가 provider usage가
   있는 경우에만 기록되는지 확인한다.
5. 표본 event에 사용자 발화, raw UUID, SQL source/model, tool/provider payload,
   token 원문이 없는지 확인한다.
6. 사용자에게 표시되는 답변, focus resource link와 run terminal 상태가 계측 전과
   동일한지 확인한다.
...
문제가 생기면 AI Worker와 App Server의 latency observer 호출을 포함한 PR을
revert한다. 이 계측은 API, DB schema, migration, Agent 저장 상태를 변경하지
않으므로 데이터 복구나 backfill은 필요 없다. rollback 뒤 기존 Agent 실행과
CloudWatch log는 그대로 유지되며 새 `agent_latency` event만 중단된다.
