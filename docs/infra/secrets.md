...
| `SQS_AGENT_JOBS_QUEUE_URL` | Agent planner queue URL |
| `SQS_MEETING_REPORT_JOBS_QUEUE_URL` | MeetingReport queue URL |
...
| `S3_UPLOADS_BUCKET` | uploads bucket name |
| `SQS_AI_JOBS_QUEUE_URL` | worker가 담당하는 단일 queue URL. 실제 병렬성은 ECS service의 desired count로 관리한다. |
| `DATABASE_SSL` | Set to `true` when `DATABASE_URL` points to Supabase/Postgres that requires SSL. |
...
| `ECS_AI_AGENT_WORKER_SERVICE` | `ecs_service_names["ai-agent-worker"]` |
