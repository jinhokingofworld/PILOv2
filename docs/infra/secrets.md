...
| `SQS_AI_JOBS_QUEUE_URL` | Agent와 MeetingReport publisher가 함께 사용하는 AI jobs queue URL |
...
| `S3_UPLOADS_BUCKET` | uploads bucket name |
| `SQS_AI_JOBS_QUEUE_URL` | `ai-worker`와 `ai-agent-worker`가 함께 소비하는 AI jobs queue URL. 실제 병렬성은 ECS service의 desired count로 관리한다. |
| `DATABASE_SSL` | Set to `true` when `DATABASE_URL` points to Supabase/Postgres that requires SSL. |
...
| `ECS_AI_AGENT_WORKER_SERVICE` | 기존 Agent Worker ECS service name. 예: `pilo-dev-ai-agent-worker` |
