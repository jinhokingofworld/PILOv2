...
- [ ] **1. 조회 기본값과 limit 계약을 고정한다.** 기간과 개수를 모두 생략한 회의록 요청은
  `createdAt DESC` 최신 1건을 선택한다. 개수가 명시되면 같은 정렬에서 정확히 해당 개수만 반환하며,
  Agent 전용 조회의 limit은 1..100 범위를 그대로 보존한다.
- [ ] **2. 상대 날짜 selector를 Workspace timezone으로 정규화한다.** 지난주는 직전 월요일 00:00부터
  이번 월요일 00:00 전까지, 다음 주는 다음 월요일 00:00부터 그 다음 월요일 00:00 전까지로 해석한다.
  최근과 “며칠 전”은 현재 시점 기준 최근 7일로, “최근 N건”은 기간보다 count와 최신 정렬을 우선한다.
- [ ] **3. 다가오는 주말 경계를 명시한다.** 주말은 Workspace timezone의 다가오는 토요일 00:00부터
  월요일 00:00 전까지로 해석한다. 토·일에 요청한 “다가오는 주말”이 현재 주말인지 다음 주말인지의
  제품 결정을 구현 전에 확정하고 regression으로 고정한다.
- [ ] **4. tool schema·resolver·service 경계를 연결한다.** `list_meeting_reports`,
  ...
  계약을 보장한다.
- [ ] **5. clarification과 multi-turn 안전 경계를 고정한다.** 0건·복수 후보·검증 불가 selector는
  UUID나 내부 오류를 노출하지 않는 clarification으로 끝내며, 후보 선택 뒤에는 이미 완료한 lookup을
  반복하지 않고 original goal을 재개한다.
- [ ] **6. catalog·eval·회귀를 추가한다.** 최신 1건, 최신 N건, 지난주, 다음 주, 최근, 며칠 전,
...
- [x] **Meeting 회의록 기본 선택** — 기간과 개수를 모두 생략한 “최근 회의록”은 `createdAt DESC` 기준
  최신 1건을 반환한다. 개수를 명시하면 기간 대신 그 개수와 최신 정렬을 우선한다.
- [x] **Meeting 상대 날짜 기본값** — “지난주”와 “다음 주”는 월요일~일요일 기준의 직전/다음 주로
  해석한다. “최근”과 “며칠 전”은 최근 7일, “주말”은 다가오는 주말로 해석한다. 경계를 정할 수 없는
  날짜 없는 “그때”는 clarification을 요청한다.
