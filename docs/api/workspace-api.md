...
- 새 Workspace는 OAuth login callback에서 server-side로 자동 생성한다.
- 새 Workspace의 `owner_user_id`는 로그인 완료된 현재 사용자에서 온다.
- MVP에서는 `owner_user_id`가 있는 Workspace를 user당 하나만 허용한다.
...
- 정상 로그인 직후 빈 배열이 반환되면 Workspace 초기화 실패로 본다.
