...
| [workspace-presence-api.md](workspace-presence-api.md) | Workspace online roster, 대표 활성 탭과 one-shot 화면 이동 Socket.IO 계약 |
| [workspace-screen-share-api.md](workspace-screen-share-api.md) | Workspace 전역 LiveKit 화면 공유 session, token, realtime 계약 |
| [chat-api.md](chat-api.md) | Workspace 일반 채팅, unread, 작성자 삭제와 멤버 멘션 알림 |
...
- [Agent Tool 개발 가이드](../AgentToolGuide.md): 도메인 service를 Agent tool adapter로
  연결하는 공용 구현·검증 규약
- [Meeting Agent Tool 매트릭스](../MeetingAgentToolMatrix.md): Meeting·MeetingReport
  사용자 의도별 현재 Agent tool과 미구현 후보의 구분
- [Meeting Agent 다단계 Workflow 설계](../MeetingAgentWorkflowDesign.md): 후속작업 자동 전달,
  evidence, 직접 회의 제어와 multi-tool Agent loop의 다음 확장 계약
- [Activity Log Registry](../ActivityLogRegistry.md): 도메인 mutation의 공용 append-only
  audit 기록 계약, action별 metadata 구조, MeetingReport 소비 경계
...
- 개인 계정 ProjectV2 원본 조회/쓰기/동기화와 Board issue create는 별도 GitHub OAuth App token(`purpose=project_v2`)을 사용한다.
