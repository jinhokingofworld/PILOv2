...
`inspect_sql_erd_schema`와 `focus_sql_erd_tables`는 모두 `riskLevel=low`인 read-only
tool이다. inspect는 session 선택 clarification을 위해 `executionMode=contextual`, focus는
`executionMode=auto`다. session, source, model, layout을 변경하지 않고 Activity Log도 남기지 않는다.
...
type FocusSqlErdTablesInput = {
  ...
  modelFingerprint: string; // inspect output 값을 그대로 전달
