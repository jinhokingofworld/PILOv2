...
- Board write API는 GitHub Issue write에는 현재 사용자의 GitHub App user OAuth token을
  사용하고, ProjectV2 item/status write에는 `/me/github/project-oauth/start`로 연결한
  ProjectV2 OAuth token(`project` scope)을 사용한다.
...
- Board write API는 GitHub Issue write를 위한 현재 사용자의 GitHub App user OAuth token이
  필요하다.
- Board issue 생성과 status 변경처럼 ProjectV2 item/status write가 필요한 API는 현재
  사용자의 ProjectV2 OAuth token(`project` scope)이 필요하다.
