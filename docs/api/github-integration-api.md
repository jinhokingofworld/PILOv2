...
GitHub App user access token이어야 하며, `repo`/`read:user`/`read:project` scope가 있는
...
- personal ProjectV2 자동 발견은 `read:project` scope가 포함된 현재 사용자의
  GitHub App user OAuth 연결이 필요하다. OAuth token이 없으면
  `GitHub user OAuth token is required for personal ProjectV2 sync`로 sync run이
  실패한다. OAuth token에 `read:project` 또는 `project` scope가 없으면
  `GitHub OAuth connection must be reconnected with read:project scope for personal ProjectV2 sync`로
...
- `POST /me/github/oauth/start` requests the GitHub OAuth scopes
  `read:user repo read:project`. Existing connections whose stored
  `users.github_token_scope` does not include `read:project` or `project` must
  be reconnected before personal ProjectV2 sync can run.
