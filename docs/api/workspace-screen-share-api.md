...
- `starting` authority는 기존 Redis TTL을 유지할 수 있다. 활성화되면 active
  authority는 조용히 만료되지 않으며, 서버는 lifecycle snapshot을 보존하고
  `startedAt`으로부터 정확히 `12 * 60 * 60`초 뒤의 durable deadline을 하나 예약한다.
- deadline worker는 due session을 lease로 claim한 뒤 기존의 compare-safe 종료 전환을
  적용한다. 종료 전환은 기존 ended realtime outbox event와 LiveKit cleanup
  stream entry가 포함된다. worker interval을 놓치거나 worker가 중단되어도 lease 만료 후
  재시도하며, 명시적 종료가 먼저 이기면 deadline 처리는 no-op이다.
...
- 화면 공유의 active authority와 lifecycle snapshot은 durable deadline 처리 전까지
  유지한다. 종료 room 분류 tombstone은 5분 유지하며 realtime outbox entry는 subscriber
  handoff 성공 시 삭제한다. LiveKit cleanup entry는 provider cleanup이 성공하거나 room/
  participant가 이미 없을 때까지 유지한다.
