...
`user_sessions` row를 생성한 뒤 frontend로 redirect한다.
...
GitHub access token을 암호화 저장한 뒤 `user_sessions` row를 생성하고
...
3. workspace가 없으면 `POST /workspaces`로 기본 workspace 생성
4. 선택 workspace id를 저장하고 `return_to` 또는 `/calendar`로 이동
