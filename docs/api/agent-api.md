...
- 선택한 resource reference가 필요한 다음 Meeting tool은 raw ID를 받지 않는다. 현재
  `start_meeting_in_room.useSelectedMeetingRoomCandidate: true`와
  `update_meeting_report_action_item.useSelectedWorkspaceMemberCandidate: true`만 해당하며, App Server가
