...
stale publisher claim을 재시도한다. 같은 sweep은 `planning` run이 120초를 넘기면 현재
...
Frontend
  -> App Server Agent API
    ...
    -> 즉시 publisher wake / 60초 recovery sweep / 120초 planning deadline
...
처리하지 않은 queued 상태를 포함한다. App Server는 60초마다 해당 run을 점검하며 120초를 넘긴
현재 turn만 `AGENT_PLANNING_TIMEOUT`으로 실패 처리한다. 이 처리에는 pending/running planner step
정리와 privacy-safe `planning_timeout` log가 포함되며, 이전 turn의 지연 SQS message는 새 turn을
terminalize할 수 없다. Frontend polling도 130초에 중단하고 사용자에게 재시도를 안내한다.
...
- publisher는 1, 2, 4, 8, 16분 backoff로 최대 5회 재시도하지만, 그 재시도는 120초 planning deadline을
  연장하지 않는다. deadline sweep이 먼저 현재 turn을 safe `failed`로 전환하고 Agent log에 운영용 event를
  남긴다.
- AI Worker planner의 infrastructure failure는 원본 SQS queue에서 세 번째 수신까지 재시도한다. Agent 전용
  SQS visibility는 90초이며, 재시도 중에도 App Server의 120초 deadline이 run과 남은 planner step을 safe
