...
- result set `generation`은 run-local `step_order`가 아니라 `runId + stepId`에서 만든 server-owned
  safe integer를 사용한다. 같은 thread의 서로 다른 run이 같은 step order를 가져도 ordinal 범위를
  합치지 않는다.
...
- resolved target은 capability chain이 끝나거나 사용자가 명시적으로 정정할 때까지
  `selectedTarget`으로 보존한다. 현재 target-aware read Tool이 없는 Drive document 후속 발화는
  `drive_context_tool_unavailable`로 fail-closed하며 Router, Planner, Tool을 호출하지 않는다.
