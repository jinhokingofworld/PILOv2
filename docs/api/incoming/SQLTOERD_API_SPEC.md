...
- request body의 `workspaceId`, `createdBy`, `updatedBy`, `tableCount`, `relationCount` 값은 신뢰하지 않는다.
- `createdBy`, `updatedBy`는 인증된 current user에서 온다.
- `tableCount`, `relationCount`는 app-server가 `modelJson`에서 계산한다.
...
MVP에서는 session 목록 endpoint를 제공하지 않는다. 추후 Workspace당 여러 sqltoerd
session을 지원할 때 plural endpoint를 별도 API 변경으로 추가한다.
...
Prisma, DBML, Mermaid, PlantUML, SQLAlchemy, Sequelize, BigQuery CTE는 MVP API에서
저장 요청을 받지 않는다.
