...
- planner는 raw tldraw shape나 draft를 만들지 않는다.
- planner는 read-only Canvas action plan만 만든다.
- shape mutation 요청은 `finish`로 현재 지원 범위를 안내한다.
...
- planner의 행동 원칙 설명
- run context, previous action, read-only action 목록을 하나의 prompt payload로 합치기

이 파일은 action 목록을 `planning/tool_catalog.py`에서 가져온다.

### `planning/tool_catalog.py`

LLM에게 허용할 read-only Canvas action 목록을 관리한다.
...
- `find_canvas_tool`은 tool-help mode에서만 노출한다.
- `connect_shapes`와 `create_draft`는 action 목록에 포함하지 않는다.
...
- 새 planner prompt에서는 이 파일을 import하거나 전달하지 않는다.
...
- shape 후보가 충분히 확실하면 `find_shapes` plan 생성
...
- LLM에게 허용할 read-only action이 바뀌면 `planning/tool_catalog.py`를 먼저 수정한다.
