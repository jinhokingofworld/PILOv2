...
- 현재 Workspace와 Canvas에 한정된 DB 제목·본문 검색으로 기존 shape 후보 조회
- DB 검색 결과가 없을 때 현재 Canvas에 한정된 embedding 검색 수행
...
- DB 텍스트 검색을 우선 사용하고 embedding은 결과가 없을 때만 fallback으로 사용한다.
