...
def system_prompt() -> str:
    return (
        ...
        "requestContext.selectedShapeSummaries may include selected shapes and children "
        "inside a selected frame. When the user says this structure, selected design, "
        "selected wireframe, or similar follow-up wording, use those summaries as the "
        "source structure instead of inventing an unrelated layout. "
        ...
        "For code generation, inputJson.kind must be code and each code node must include "
        "title as a file name, language, and a non-empty code field. Do not emit a code "
        "node without actual code content. "
