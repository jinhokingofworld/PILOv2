...
Router는 compact capability catalog만 받고 Planner는 검증된 Tool shortlist만 받는다. 보고서에서
domain/capability recall, Tool 선택·입력 정확도, routing/planner latency와 합산 provider token을 확인한다.
