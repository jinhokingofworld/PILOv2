...
CI는 provider를 호출하기 전에 `prompt_injection_security_gate_v1.json`의 사용자 발화와 bounded thread
resource fixture를 실행한다. 명시적인 system instruction override, system prompt·credential 추출,
shortlist·registry·ECS flag 변경, confirmation·권한 우회 신호는 `prompt_injection_suspected`로 차단한다.
보안 주제를 정상적으로 논의하거나 기존 confirmation을 요청하는 인접 반례는 차단하지 않는다.
