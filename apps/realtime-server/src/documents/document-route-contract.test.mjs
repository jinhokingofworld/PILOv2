...
test("routes only document sync upgrades through the Hocuspocus transport", async () => {
  ...
  const hocuspocusService = await readFile(
    new URL("./document-hocuspocus.service.ts", import.meta.url),
    "utf8",
  );

  assert.match(server, /createDocumentHocuspocusService/);
  assert.match(server, /createDocumentCheckpointService/);
  assert.match(server, /createDocumentAppServerClient/);
  ...
  assert.match(server, /await documentHocuspocusService\.shutdown\(\)/);
