...
import { createBoardIssueLoadPublicationGuard } from "./utils/board-issue-load-publication-guard.ts";
...
test("keeps the first page renderable while a later page is deferred", async () => {
  let resolveLaterPage;
  let boardStatus = "loading";
  let publishedFirstPage = [];
  const laterPage = new Promise((resolve) => {
    resolveLaterPage = resolve;
  });

  const loading = loadAllBoardIssuePages({
    fetchPage: async (requestedPage) => {
      if (requestedPage === 1) return page([issue(1)], 200);
      return laterPage;
    },
    onFirstPage: (firstPage) => {
      publishedFirstPage = firstPage.data;
      boardStatus = "success";
    }
  });

  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(boardStatus, "success");
  assert.deepEqual(publishedFirstPage.map((item) => item.id), ["1"]);

  resolveLaterPage(page([issue(2)], 200));
  await loading;
});

test("rejects late page publications after mutation invalidation", async () => {
  const guard = createBoardIssueLoadPublicationGuard();
  const canPublish = guard.begin();
  let published = false;
  const latePage = Promise.resolve().then(() => {
    if (canPublish()) published = true;
  });
