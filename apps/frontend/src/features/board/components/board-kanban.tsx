  ...
  return (
    ...
    >
      <span className="card-top flex items-center justify-between gap-3">
        ...
        <span
          className={cn(
            "state inline-flex min-h-[22px] items-center rounded-full px-2 text-[10.5px] font-extrabold",
      ...
      <strong className="card-title line-clamp-2 text-[15.5px] font-bold leading-snug tracking-normal">
        {issue.title}
      </strong>

      {visibleLabels.length ? (
        <span className="tags flex min-h-6 flex-wrap items-center gap-1.5">
          {visibleLabels.map((label) => (
            <span
              key={label.name}
              className="tag inline-flex min-h-[22px] max-w-32 items-center truncate rounded-full border border-slate-200 bg-slate-50 px-2 text-[10.5px] font-bold text-slate-600"
              style={{
      ...
      <span className="card-footer mt-auto flex items-center justify-between gap-2 text-[11.5px] text-slate-400">
        <span className="avatars flex items-center">
          {visibleAssignees.length ? (
            visibleAssignees.map((login) => (
              <span
                key={login}
                className="avatar -ml-1.5 first:ml-0 grid size-6 place-items-center rounded-full border-2 border-white bg-violet-500 font-mono text-[9.5px] font-extrabold text-white"
                title={`@${login}`}
              >
      ...
      <div className="kanban-scroll overflow-x-auto p-6">
        <div className="kanban-board grid min-w-[1060px] grid-cols-5 gap-3.5">
          {[0, 1, 2, 3, 4].map((index) => (
            ...
            >
              <div className="lane-header flex min-h-[54px] items-center justify-between gap-2 border-b border-slate-200 px-3 py-3">
                <div className="h-4 w-28 animate-pulse rounded bg-slate-200" />
                <div className="h-6 w-8 animate-pulse rounded-full bg-slate-200" />
              </div>
              <div className="lane-stack grid gap-2.5 p-3">
                <div className="h-24 animate-pulse rounded-[14px] bg-slate-200" />
                <div className="h-24 animate-pulse rounded-[14px] bg-slate-200" />
              </div>
            </article>
          ))}
            ...
            >
              <header className="lane-header flex min-h-[54px] items-center justify-between gap-2 border-b border-slate-200 px-3 py-3">
                <div className="lane-name flex min-w-0 items-center gap-2">
                  ...
                  <div className="min-w-0">
                    <h2 className="truncate text-[14.5px] font-bold leading-tight">
                      {column.name}
                    </h2>
                    <span className="block truncate text-[11.5px] font-medium text-slate-400">
                      ProjectV2 Status
                    </span>
                  </div>
                </div>
                <span className="lane-count inline-flex h-6 min-w-7 items-center justify-center rounded-full border border-slate-200 bg-white px-2 font-mono text-[11px] font-bold text-slate-500">
                  {columnIssues.length}
                </span>
              </header>
              ...
              <div className="lane-stack grid gap-2.5 p-3">
                {columnIssues.length ? (
                  ...
                  <div className="empty-slot grid min-h-[86px] place-items-center rounded-[14px] border border-dashed border-slate-300 bg-white/50 px-3 text-center text-xs font-bold text-slate-400">
