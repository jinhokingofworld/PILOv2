...
새 head를 감지한 기존 버전은 참고용으로 계속 표시하되 읽기 전용으로 잠근다. 이 상태에서는
file decision 저장, Canvas 편집, Conflict 해결안 적용, Review 제출과 Merge를 막고, 사용자가
`최신 버전 분석 시작`을 눌러야 새 revision 분석을 시작한다.
...
- 이전 session의 분석 결과와 제출 이력은 내부 추적 데이터로만 보존한다.
- 초기 UX에서는 이전 버전 목록이나 진입점을 제공하지 않고, 사용자에게는 최신 버전 하나만
  표시한다.
- 새 head를 감지했을 때만 기존 화면을 임시 read-only 상태로 유지해 최신 버전 분석 시작을
  안내한다.
...
- [x] review Canvas room join/access check 추가
- [x] presence와 cursor 공유 검증
- [ ] operation broadcast/catch-up 검증
- [ ] decision optimistic concurrency와 409 처리
- [ ] progress/node badge realtime 갱신

### 4-F Sidebar와 Room Lifecycle UX

- [x] `리뷰할 PR` / `리뷰 공간` 하위 메뉴 구현
- [x] 시작, 합류, 분석 중, 새 커밋 분석 시작 CTA 구현
- [x] 이전 버전 목록 없이 최신 버전만 조작하는 UX 정책 확정
- [x] merge/close read-only 전환과 reopen 처리
...
- [x] merge/close 후 모든 접속자가 read-only 상태를 본다.
