...
| --- | --- | --- |
| Auth | 동현 | Google/GitHub login OAuth, bearer session, logout, auth rule |
| Settings | 동현 | user profile override, personal settings, account lifecycle |
...
| sqltoerd | 세인 | MVP session, SQL DDL parsing, ERD model, table card, FK relation line, Inspector, Workspace session API |
...
- `AGENTS.md`, `convention.md`, `coding-rule.md`처럼 팀 전체 작업 규칙을 바꾸는 문서 변경은 사이렌 변경으로 본다.
